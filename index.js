...
/********SETTING HERE**********/
const ownerNumber = ["6281539336834@s.whatsapp.net"] // ganti nomor kamu
const vhterapi = 'loplopia' // api key vhtear
const tobz = 'BotWeA' // api key tobz
const vinz = 'apivinz' // api key zeks
const apiKey = 'Nkwp9oR7Eg9yyKbejBpm' // api key mhankbarbar
/************************************/
...
client.on('group-participants-update', async (anu) => {
		...
		try {
			...
			if (anu.action == 'add') {
				...
				teks = `Hallo Kamu @${num.split('@')[0]}\n Selamat datang di group *${mdata.subject}* Semoga Netah ^_°`
	...
	client.on('message-new', async (mek) => {
		try {
           ...
           return false
			...
			switch(command) {
				...
				if ( checkATMuser(sender) >= total ) {
					...
					await reply(`*✔️Pembayaran Berhasil✔️*\n\n*pengirim* : Admin\n*penerima* : ${pushname}\n*nominal pembelian* : ${payout} \n *harga limit* : ${koinPerlimit}/limit\n *sisa uang mu* : ${checkATMuser(sender)}\n\nproses berhasil dengan nomer pembayaran\n${createSerial(15)}`)
				} 
				break
				case 'limit':
				   if (!isRegistered) return reply(ind.noregis())
			    ...
			    link = body.slice(6)
			    reply(ind.wait())
			    anu = await fetchJson(`http://arugaz.my.id/api/media/ytaudio?url=${link}`, {method: 'get'})
			    pp = await getBuffer(anu.result.dl_link)
			   auf = await getBuffer(anu.result.UrlMp3)
			   chat = '*Link Di Temukan*\n*⊱TITLE*:${anu.result.title}\n*⊱SIZE*${anu.result.filesizeF}\n*SEDANG MENDOWNLOAD MP3 MOHON JANGAN SPAM (:*'
			   client.sendMessage(from, pp, image, {quoted: mek, caption: chat})
			   client.sendMessage(from, auf, audio, {mimetype: 'audio/mp4', quoted: mek})
			   break
				case 'vinta':
				    if (!isRegistered) return reply(ind.noregis())
				if (isLimit(sender)) return reply(ind.limitend(pusname))
                ...
                case 'beritahoax':
					...
					gol = body.slice(7)
					reply(ind.wait())
					anu = await fetchJson(`https://api.vhtear.com/googlesearch?query=${gol}&apikey=${vhterapi}`, {method: 'get'})
					teks = '=================\n'
					for (let tod of anu.result) {
						teks += `${tod}\n=================\n`
						}
						reply(teks.trim())
						await limitAdd(sender)
					...
					ttp = body.slice(8)
					reply(ind.wait())
					anu = await getBuffer(`https://arugaz.my.id/api/textpro/thundertext?text=${ttp}`)
					...
					reply(ind.wait())
					pla = body.slice(10)
					anu = await fetchJson(`https://api.vhtear.com/playstore?query=${pla}&apikey=${vhterapi}`, {method: 'get'})
						bb = await getBuffer(anu.result.icon)
						teks = '*⊱NAMA*:${anu.result.title}\n*⊱DESCRIPSI*${anu.result.description}\n*⊱DEVELOPER*:${anu.result.developer}\n*⊱PRICE*:${anu.result.price}\n=================\n'
						client.sendMessage(from, bb, image, {quoted: mek, caption: teks})
						await limitAdd(sender)
						break 
					case 'emoji':
					if (!isRegistered) return reply(ind.noregis())
					...
					tof = body.slice(6)
					reply(ind.wait())
					gg = await getBuffer(`https://api.zeks.xyz/api/emoji-image?apikey=${vinz}&emoji=${tof}`)
					client.sendMessage(from, gg, image, {quoted: mek, caption: 'nih'})
					await limitAdd(sender)
					break 
					case 'wiki':
					if (!isRegistered) return reply(ind.noregis())
					if (isLimit(sender)) return reply(ind.limitend(pusname)) 
					if (args.length < 1) return reply(`teks nya mana un?`)
					wik = body.slice(5)
					anu = await fetchJson(`https://api.zeks.xyz/api/wiki?q=${wik}&apikey=${vinz}`, {method: 'get'})
					teks = `*[❗] Ini Yang Saya Temukan Di Wikipedia*:\n\n*${anu.result.result}*`
					reply(teks)
					await limitAdd(sender)
					break 
					case 'nomorhoki':
					...
					if (args.length < 1) return reply(`teks nya mana um?`)
					no = body.slice(10)
					anu = await fetchJson (`https://api.vhtear.com/nomerhoki?no=${no}&apikey=${vhterapi}`, {method: 'get'})
					...
					if (isLimit(sender)) return reply(ind.limitend(pusname)) 
					reply(ind.wait())
					anu = await fetchJson(`https://api.vhtear.com/randomcat?apikey=${vhterapi}`, {method: 'get'})
					cing = await getBuffer(anu.result.url)
					client.sendMessage(from, cing, image, {quoted: mek, caption: 'Meong☺️'})
					break 
					case 'loli':
					if (!isRegistered) return reply(ind.noregis())
					if (isLimit(sender)) return reply(ind.limitend(pusname)) 
					anu = await fetchJson(`https://api.vhtear.com/randomloli&apikey=${vhterapi}`, {method: 'get'})
					nime = await getBuffer(anu.result.url)
					client.sendMessage(from, nime, image, {quoted: mek, caption: 'loli'})
					await limitAdd(sender)
					break 
					case 'nekonime':
					if (!isRegistered) return reply(ind.noregis())
					if (isLimit(sender)) return reply(ind.limitend(pusname)) 
					anu = await fetchJson(`https://api.vhtear.com/randomnekonime&apikey=${vhterapi}`, {method: 'get'})
					nime = await getBuffer(anu.result.url)
					client.sendMessage(from, nime ,image, {quoted: mek, caption: 'neko'})
					await limitAdd(sender)
					break 
					case 'randomnime':
					if (!isRegistered) return reply(ind.noregis())
					if (isLimit(sender)) return reply(ind.limitend(pusname)) 
					reply(ind.wait())
					anu = await fetchJson(`https://api.vhtear.com/randomwibu&apikey=${vhterapi}`, {method: 'get'})
					ttp = await getBuffer(anu.result.foto)
					teks = `*NAMA*:${anu.result.nama}\n*DESCRIPTION*:${anu.result.deskripsi}\n\n*SOURCE*:${anu.result.sumber}`
					client.sendMessage(from, ttp, image, {quoted: mek, caption: teks})
					break 
					case 'ytsearch':
					if (!isRegistered) return reply(ind.noregis())
					if (isLimit(sender)) return reply(ind.limitend(pusname)) 
					sit = body.slice(9)
					reply(ind.wait())
					anu = await fetchJson(`https://api.vhtear.com/ytmp3?query=${sit}&apikey=${vhterapi}`, {method: 'get'})
					ee = await getBuffer(anu.result.image)
					ep = await getBuffer(anu.result.mp3)
					text = `*DATA DI TEMUKAN!!*\n*⊱TITLE*:${anu.result.title}\n*⊱FILESIZE*:${anu.result.filesize}\n*⊱DURASI*${anu.result.duration}\n*SEDANG MENDOWNLOAD MP3*`
					client.sendMessage(from, ee, image ,{quoted: mek, caption: text})
					client.sendMessage(from, ep, audio, {mimetype: 'mp4/audio', quoted: mek})
					await limitAdd(sender)
					break
					case 'brainly':
					if (!isRegistered) return reply(ind.noregis())
					if (isLimit(sender)) return reply(ind.limitend(pusname))
                    bruh = body.slice(8)
                    anu = await fetchJson(`https://api.vhtear.com/branly?query=${bruh}dunia&apikey=${vhterapi}`, {method: 'get'})
                    teks = `*[❗] Ini Yang Saya Temukan Di Brainly*\n\n${anu.result.data}`
                    reply(teks)
					await limitAdd(sender)
					break 
				case 'bcgc':
					...
					if (isMedia && !mek.message.videoMessage || isQuotedImage) {
						...
						for (let _ of anu) {
							client.sendMessage(_.jid, buff, image, {caption: `*「🥶FROZE🥶」*\n\nDari Grup : ${groupName}\nPengirim : wa.me/${(sender.split('@')[0])}\nPesan : ${body.slice(6)}`})
						}
						reply('')
					} else {
						for (let _ of anu) {
							sendMess(_.jid, `*「🥶FROZE🥶」*\n\nDari Grup : ${groupName}\nPengirim : wa.me/${(sender.split('@')[0])}\nPesan : ${body.slice(6)}`)
						}
					...
					client.updatePresence(from, Presence.composing) 
					hasp = body.slice(10)
					data = await fetchJson(`https://api.vhtear.com/pinterest?query=${hsp}&apikey=${vhterapi}`, {method: 'get'})
					reply(ind.wait())
					n = JSON.parse(JSON.stringify(data.result));
					nimek =  n[Math.floor(Math.random() * n.length)];
					pok = await getBuffer(nimek)
					client.sendMessage(from, pok, image, { quoted: mek, caption: `*⚠️INI YANG SAYA TEMUKAN⚠️*`})
					await limitAdd(sender)
					break 
					case 'resepmasakan':
					if (!isRegistered) return reply(ind.noregis())
                   ...
                   await limitAdd(sender)
					...
					if (!isRegistered) return reply(ind.noregis())
					if (isLimit(sender)) return reply(ind.limitend(pusname))
                if (args.length < 1) return reply('teks nya mana um kalo om gk tau list tinta ketik\n${prefix}listtinta')
                reply(ind.wait())
                sip = body.slice(6)
                var nam = sip.split("|")[0];
                var kel = sip.split("|")[1];
                var tek = sip.split("|")[2];
                var tin = sip.split("|")[3];
                anu = await getBuffer(`https://api.zeks.xyz/api/magernulis?nama=${nam}&kelas=${kel}&text=${tek}&tinta=${tin}`)
                client.sendMessage(from, anu, image, {quoted: mek, caption: 'nih'})
                await limitAdd(sender)
                break 
                case 'apkpure':
				...
				reply(ind.wait())
				teks1 = '=================\n'
				apk = await fetchJson(`https://api.zeks.xyz/api/apkpure?q=${pk}&apikey=${vinz}`, {method: 'get'})
				...
				break
                   ...
                   if (isLimit(sender)) return reply(ind.limitend(pusname))
					...
					if (!isRegistered) return reply(ind.noregis())
					client.updatePresence(from, Presence.composing) 
					teks = `\`\`\`Ini adalah list grup teman owner:\n\n\`\`\``
					no = 0
		  ...
		  case 'tahta':
           ...
           lop = body.slice(7)
           reply(ind.wait())
           anu = await getBuffer(`https://api.zeks.xyz/api/hartatahta?text=${body.slice(7)}&apikey=${vinz}`)
           client.sendMessage(from, anu, image, {quoted: mek, caption: '*Harta Tahtah*:'+ lop})
           await limitAdd(sender)
           break
          case 'speed':
          case 'ping':
            ...
            await client.sendMessage(from, `Pong!!!!\nSpeed: ${processTime(time, moment())} _Second_`)
               ...
               client.sendMessage(from, resul, text, { quoted: mek})
				...
				if (!isRegistered) return reply(ind.noregis())
					if (isLimit(sender)) return reply(ind.limitend(pusname)) 
					if (args.length < 1) return reply(`teks nya mana um?`)
					virtex = body.slice(10)
					reply(ind.wait())
					anu = await fetchJson(`https://api.vhtear.com/infomobil?merk=${virtex}&apikey=${vhterapi}`, {method: 'get'})
					em = await getBuffer(anu.result.image)
					bim = `*⊱MERK*:${anu.result.title}\n*⊱HARGA*:${anu.result.harga}\n*⊱SPESIPIKASI*:\n${anu.result.spesifikasi}\n*⊱KELEBIHAN*:${anu.result.kelebihan}\n*⊱KEKURANGAN*:${anu.result.kekurangan}`
					client.sendMessage(from, em, image, {quoted: mek, caption: bim})
					await limitAdd(sender)
					break 
				case 'infomotor':
				if (!isRegistered) return reply(ind.noregis())
					...
					virtex = body.slice(10)
					reply(ind.wait())
					anu = await fetchJson(`https://api.vhtear.com/infomotor?merk=${virtex}&apikey=$vhterapi}`, {method: 'get'})
					em = await getBuffer(anu.result.image)
					bim = `*⊱MERK*:\n${anu.result.title}\n*⊱HARGA*:\n${anu.result.harga}\n*⊱SPESIPIKASI*:\n${anu.result.spesifikasi}\n*⊱KELEBIHAN*:\n${anu.result.kelebihan}\n*⊱KEKURANGAN*:\n${anu.result.kekurangan}\n`
					client.sendMessage(from, em, image, {quoted: mek, caption: bim})
					await limitAdd(sender)
					break
                case 'quotemaker':
			...
			if (isLimit(sender)) return reply(ind.limitend(pusname))
				...
				if (!isRegistered) return reply(ind.noregis())
					if (isLimit(sender)) return reply(ind.limitend(pusname))
                                        ...
                                        ranp = getRandom('.gif')
                                        rano = getRandom('.webp')
                                        anu1 = `https://api.zeks.xyz/api/triger?apikey=${teks}&img=${teks}`
